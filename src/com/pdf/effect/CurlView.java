...
//////	
	...
	public void onDrawFrame() {
		...
		if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
			...
			MuPDFActivity.handler.sendMessage(msgObj);
			//bigant end
	...
	public static final int k_DummySate = -1;
	public static final int k_ZoomState = 0;
	public static final int k_EffectState = 1;
	public int mState = k_DummySate;
	

	private RectF rightRect;
	private RectF leftRect;

	public boolean onTouchEffectDown1(MotionEvent me){
		...
		if(delta >= k_delta){
			...
			//DocumentReaderView.s_Instant.setVisibility(INVISIBLE);
	...
	public boolean onTouchEffectMove(MotionEvent me){
		updateCurlPos(mPointerPos);
		return true;
	}
	...
	// nhi add to control buttons list in menu	
		...
		if( me.getActionMasked() == MotionEvent.ACTION_DOWN )
		{
			if (mViewMode == SHOW_TWO_PAGES) 
			{
				if( me.getX() >= MuPDFActivity.PHONE_WIDTH/3 && me.getX() <= MuPDFActivity.PHONE_WIDTH*2/3)
				{
					onTapMainDocArea();
					return false;
				}
			...
			{
				if( me.getX() >= MuPDFActivity.PHONE_WIDTH/4 && me.getX() <= MuPDFActivity.PHONE_WIDTH*3/4)
				{
					onTapMainDocArea();
					return false;
				}
